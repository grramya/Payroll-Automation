...
def _compute_period(m: pd.DataFrame) -> dict:
    ...
    # Unclassified residual — total P&L amount minus all amounts captured above
    matched_total = rev_total + cogs_total + opex_total + da + oi + oe + tax
...
_MONTH_ABB_PL = {"Jan":"01","Feb":"02","Mar":"03","Apr":"04","May":"05","Jun":"06",
                 "Jul":"07","Aug":"08","Sep":"09","Oct":"10","Nov":"11","Dec":"12"}

def _month_to_ym_pl(m: str) -> str:
    try:
        mon, yr = m.split("-")
        full_yr = f"20{yr}" if len(yr) == 2 else yr
        return f"{full_yr}-{_MONTH_ABB_PL[mon]}"
    except Exception:
        return "0000-00"

def _derive_quarter(m: str) -> str:
    try:
...
def _build_comp_pl_excel(
    data: dict, months: list, quarters: list, years: list, company_name: str
) -> bytes:
    """Render the Comparative P&L (Class) Excel from pre-aggregated data."""
    wb = Workbook()
    ws = wb.active
...
def run_comparative_pl(df: pd.DataFrame, company_name: str) -> bytes:
    """Generate the Comparative P&L (Class) Excel from a raw DataFrame."""
    data, months, quarters, years = _aggregate(df)
    return _build_comp_pl_excel(data, months, quarters, years, company_name)


def _quarter_in_range(q: str, from_ym: str | None, to_ym: str | None) -> bool:
    """Return True if quarter q overlaps with the [from_ym, to_ym] date range."""
    try:
        q_num = int(q[1])
        yr    = q[3:]
...
def _four_quarters_ending_at(q: str) -> list[str]:
    """Return 4 consecutive quarters ending at q (inclusive), oldest first."""
    try:
        q_num = int(q[1])
        year  = int(q[3:])
    except (ValueError, IndexError):
...
def _build_comp_pl_ui_excel(
    ...
    """Generate Comparative P&L Excel matching the UI column layout exactly.

    Columns: all filtered_months | four_quarters | [selected_year-1, selected_year]
    """
    ...
    # Only include groups that have at least one column
    ...
    ws["A1"].font = F(bold=True, size=14, color="0F172A"); ws.row_dimensions[1].height = 22
    ws.append(["Comparative P&L (Class)"] + [None] * n)
    ws["A2"].font = F(bold=True, size=11, color="1E3A5F")
    ws.append([f"As of {as_of}"] + [None] * n)
    ws["A3"].font = F(size=9, color="64748B", italic=True)
    ws.append([None] * (n + 1))  # spacer

    # ── Group header row (row 5) ──────────────────────────────────────────────
    group_row = [None] * (n + 1)
    col_idx = 1  # 1-based label column
    for lbl, cols in groups:
        group_row[col_idx] = lbl   # first column of each group
        col_idx += len(cols)
    ws.append(group_row)
    gr = ws.max_row
    ws.row_dimensions[gr].height = 14
    # Style group headers and merge each group span
    col_offset = 2  # 1-based, skip label col A
    for lbl, cols in groups:
        if not cols:
            continue
    ...
    for rdef in rows_def:
        ...
        if key is None:
            ...
            vals = [data.get(p, {}).get(key) for p in period_cols]
...
def run_comparative_pl_from_preview(
    preview: dict,
    from_ym: str | None,
    to_ym: str | None,
    ...
    """Generate a date-filtered Comparative P&L (Class) Excel matching the UI exactly."""
    ...
    # Filter months by date range (same logic as UI)
    filtered_months = [
        m for m in all_months
        if (not from_ym or _month_to_ym_pl(m) >= from_ym)
        and (not to_ym   or _month_to_ym_pl(m) <= to_ym)
    ]
    if not filtered_months:
        filtered_months = all_months

    # Trust the quarter the UI computed and sent; fall back only when nothing was sent.
    filtered_quarters = [q for q in all_quarters if _quarter_in_range(q, from_ym, to_ym)]
    if selected_quarter:
    ...
    four_quarters = _four_quarters_ending_at(effective_quarter)

    # Determine selected year
    if selected_year is None:
        selected_year = all_years[-1] if all_years else datetime.now().year

    return _build_comp_pl_ui_excel(
