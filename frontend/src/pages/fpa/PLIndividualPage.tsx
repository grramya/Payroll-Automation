    ...
    <Box className="page-enter">
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, flexWrap: "wrap" }}>
              {months.length > 0 && (
                <FormControl size="small" sx={{ minWidth: 130 }}>
                  ...
                  <Select labelId="pli-month-label" value={selectedMonth} label="As of" onChange={(e: SelectChangeEvent) => setSelectedMonth(e.target.value)}>
      ...
      <Container maxWidth="xl" sx={{ py: 3 }}>
        {plPreview && months.length > 0 ? (
          <FullScreenWrapper title={`Base P&L — Class (Individual) — ${companyName} — ${selectedMonth}`} fullContent={<PLIndividualTable data={plPreview as never} selectedMonth={selectedMonth} companyName={companyName} />}>
            <PLIndividualTable data={plPreview as never} selectedMonth={selectedMonth} companyName={companyName} />
          </FullScreenWrapper>
        ) : (
          <div role="status" className="empty-state">
            No P&amp;L data found. Ensure the uploaded file contains Profit &amp; Loss accounts with Classification 2 mappings.
          </div>
        )}
      </Container>
    </Box>
