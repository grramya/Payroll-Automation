...
const fmt = (v: number | null | undefined, isMetric: boolean): string =>
  (isMetric ? fmtPct(v) : fmtAmt(v));
...
// ── Row definition (from compPlPreview.rows) ──────────────────────────────────
...
export default function ComparativePLPage() {
  ...
  const { compPlPreview, companyName } = result;
  ...
  const setFromDate        = (v: Dayjs | null) => setPageFilter("compPL", { fromDate: v, toDate, selectedQuarter, selectedYear });
  const setToDate          = (v: Dayjs | null) => setPageFilter("compPL", { fromDate, toDate: v, selectedQuarter, selectedYear });
  const setSelectedQuarter = (v: string)       => setPageFilter("compPL", { fromDate, toDate, selectedQuarter: v, selectedYear });
  const setSelectedYear    = (v: number)       => setPageFilter("compPL", { fromDate, toDate, selectedQuarter, selectedYear: v });
  ...
  const fourQuarters = useMemo<string[]>(
  ...
  const handleDownload = async () => {
    ...
    const blob = await fpaDownloadFilteredReport("comp_pl", {
      from_ym: fromYM,
      to_ym:   toYM,
    ...
    a.href = url; a.download = `${companyName}_comparative_pl.xlsx`; a.click();
    ...
    <Box className="page-enter">
      ...
      <Container maxWidth="xl" sx={{ py: 3, px: { xs: 1, md: 3 } }}>
        ...
        <Box
          ...
          sx={{ overflow: "auto", maxHeight: "calc(100vh - 220px)", borderRadius: 2, border: "1px solid #E2E8F0", boxShadow: 1 }}
        >
          <table
            aria-label={tableCaption}
            style={{
