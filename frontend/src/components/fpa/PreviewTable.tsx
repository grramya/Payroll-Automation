...
export default function PreviewTable({ rows }: Props) {
  ...
  const fmtCell = (col: string, val: unknown): React.ReactNode => {
  ...
  return (
    <Paper elevation={0} variant="outlined" sx={{ borderRadius: 3, overflow: "hidden" }}>
      ...
      <TableContainer sx={{ overflow: "auto", maxHeight: "calc(100vh - 220px)" }}>
              ...
              {VISIBLE_COLS.map((col) => (
                <TableCell
                  ...
                  sx={{
                    ...
                    whiteSpace: "nowrap", py: 1.25, textTransform: "uppercase",
                  }}
                >
                  {col}
                </TableCell>
            ...
            {pageRows.map((row, i) => (
              ...
              >
                {VISIBLE_COLS.map((col) => (
                  <TableCell
                    key={col}
                    sx={{ whiteSpace: "nowrap", color: "text.secondary", py: 1, borderBottom: "1px solid #F1F5F9" }}
                  >
                    {fmtCell(col, row[col])}
                  </TableCell>
                ))}
